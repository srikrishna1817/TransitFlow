...
st.caption("Generate, download, and manage professional PDF reports for operations and executive review.")
st.divider()
...
with tab1:
    if not REPORT_OPTIONS:
        st.warning("⚠️ You do not have permission to generate new reports.")
    else:
        ...
        with col2:
            st.info("💡 **Tip:** PDFs are automatically stamped with project branding and saved to the local `reports/` folder.")
...
with tab2:
    ...
    if df_history.empty:
        st.info("ℹ️ No reports generated yet.")
    else:
        # Simple display table
        display_df = df_history.copy()
        display_df['generated_at'] = pd.to_datetime(display_df['generated_at'])
        
        # "Last generated X minutes ago" label
        now_ts = datetime.datetime.now()
        display_df['_mins_ago'] = ((now_ts - display_df['generated_at']).dt.total_seconds() / 60).round(0).astype(int)
        display_df['Last Generated'] = display_df['_mins_ago'].apply(
            lambda m: f"{m} min ago" if m < 60 else f"{m//60} hr ago"
        )
        ...
        display_df = display_df[['report_type', 'report_date', 'generated_at', 'username', 'Size', 'Last Generated', 'report_id', 'file_path']]
        display_df.columns = ['Report Type', 'Report Date', 'Generated At', 'Generated By', 'Size', 'Last Generated', 'ID', 'Path']
        
        st.dataframe(
            display_df[['ID', 'Report Type', 'Report Date', 'Generated At', 'Generated By', 'Size', 'Last Generated']].head(10),
            hide_index=True, use_container_width=True
        )
        with st.expander("Show all reports"):
            st.dataframe(
                display_df[['ID', 'Report Type', 'Report Date', 'Generated At', 'Generated By', 'Size', 'Last Generated']],
                hide_index=True, use_container_width=True
            )
...
render_page_nav('pages/08_🚇_Simulation.py', '🚇 Simulation', 'pages/10_⚙️_Settings.py', '⚙️ Settings')
